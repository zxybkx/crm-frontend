...
import {Form, Input, Row, Col, Button, Divider, Popconfirm, Select, message} from 'antd';
...
export default class AddPgb extends PureComponent {
    ...
    validateFields((errs, fields) => {
      ...
      if (id) {
        ...
        dispatch({
          ...
          dispatch(
            routerRedux.push({
              pathname: '/setting/pgbManage/pgbmb',
              query: {
                id: data.id,
              }
            }),
  ...
  render() {
    ...
    const {breadcrumbs, pgbTypes, pfxData, pfbData} = this.state;
    ...
    return (
      <PageLayout className={styles.default} breadcrumb={breadcrumbs}>
        <div className={styles.addDiv}>
          <Row>
            {/*<Col span={12}>*/}
            {/*  <FormItem label={'评估表类型：'} labelCol={{span: 6}} wrapperCol={{span: 6}}>*/}
            {/*    {getFieldDecorator('type', {*/}
            {/*      initialValue: pfbData && pfbData.type,*/}
            {/*      rules: [{required: true, message: '请选择评估表类型'}],*/}
            {/*    })(*/}
            {/*      <Select style={{width: 200}} placeholder="评估表类型" allowClear>*/}
            {/*        {*/}
            {/*          pgbTypes && pgbTypes.map(o => <Option key={o.useName}>{o.name}</Option>)*/}
            {/*        }*/}
            {/*      </Select>,*/}
            {/*    )}*/}
            {/*  </FormItem>*/}
